...
interface CaregiverProfileForm {
  full_name: string
  phone: string
  relationship: string
}
...
export function CaregiverProfilePage() {
  ...
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<CaregiverProfileForm>({
    values: {
  ...
  async function onSubmit(values: CaregiverProfileForm) {
    try {
  ...
  if (!profile) return <div className="flex justify-center py-20"><Spinner size={28} className="text-sage" /></div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-2xl">
        ...
        <CardBody>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-light capitalize">Caregiver</p>
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <Input label="Relationship to Patient" placeholder="e.g. Spouse, Child, Professional Carer" {...register('relationship')} />
        </CardBody>
      </Card>
