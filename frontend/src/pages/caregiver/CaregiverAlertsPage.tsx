...
export function CaregiverAlertsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader title="All Alerts" subtitle="Missed doses, low stock, critical readings" />
        <CardBody>
          {notifications.length === 0 ? (
            <EmptyState
              icon="🔔"
              title="No alerts"
              description="You'll see missed doses, low stock alerts, and critical vitals here."
            />
          ) : (
            <div className="divide-y divide-border">
              {notifications.map(n => (
                <div
                  key={n.id}
                  className={`flex items-start gap-3 py-3 first:pt-0 last:pb-0 cursor-pointer transition-colors ${
                    n.is_read ? '' : 'hover:bg-sage/5'
                  }`}
                  onClick={() => !n.is_read && markAsRead.mutate(n.id)}
                >
                  <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${
                    n.is_read ? 'bg-transparent' : 'bg-sage'
                  }`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      <p className={`text-sm font-medium ${ n.is_read ? 'text-slate-light' : 'text-ink' }`}>{n.title}</p>
                      <Badge label={TYPE_LABEL[n.type] ?? n.type} variant="slate" />
                      <Badge
                        label={SEVERITY_BADGE[n.severity]?.label ?? n.severity}
                        variant={SEVERITY_BADGE[n.severity]?.variant ?? 'slate'}
                        dot
                      />
                    </div>
                  ...
                  {!n.is_read && <span className="text-[11px] text-sage font-medium shrink-0 mt-1">Tap to mark read</span>}
                </div>
              ))}
            </div>
          )}
